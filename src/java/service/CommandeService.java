...
public class CommandeService implements IDao<Commande>{
    ...
    public List<Commande> findByClient(Client c){
        ...
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            commandes  =  session.createQuery("from Commande cd where cd.client = "+c.getId()).list();
            tx.commit();
        } catch (HibernateException e) {
            if(tx != null)
                tx.rollback();
    ...
    public List<Commande> findByFacture(Facture f){
        ...
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            commandes  =  session.createQuery("from Commande cd where cd.facture = "+f.getId()).list();
            tx.commit();
        } catch (HibernateException e) {
            if(tx != null)
                tx.rollback();
